    ...
    public class SQLHelpDataBase
    {
        ...
        public DataSet ExecuteSql(string strSql)
        {
            lock (vExecuteSql1)
            {
                ...
                try
                {
                    using (OracleConnection dbcon = new OracleConnection(strCon))
                    {
                        if (dbcon.State == ConnectionState.Closed)
                        {
                            dbcon.Open();
                        }
                        OracleCommand cmd= new OracleCommand(strSql,dbcon);
        ...
        /// <param name="parameters"></param>
        /// <returns></returns>
        object vRunProcedure = new object();
        public DataSet RunProcedure(string stroreProcName, IDataParameter[] parameters,out string sg)
        {
            lock (vRunProcedure)
            {
                sg = string.Empty;
                ...
                try
                {
                    using (this.dbconold)
                    {
                        if (dbconold.State == ConnectionState.Closed)
                        {
                            dbconold.Open();
                        }
                        DbDataAdapter dbada = objProviderFactory.CreateDataAdapter();
                        dbada.SelectCommand = BuildCommand(this.dbconold, stroreProcName, parameters);
                        dbada.Fill(ds);
                        this.dbconold.Close();
                    }
                }
                ...
                {
                    sg = string.Format("异常:[{0}{1}][{2}]", sErrCode, new StackTrace(new StackFrame(true)).GetFrame(0).GetFileLineNumber(), ex.Message);
                }
                return ds;
            }
        }
        ...
        public void RunProcedure(string stroreProcName, IDataParameter[] parameters, out System.Collections.Hashtable ht)
        {
            ...
            try
            {
                using (this.dbconold)
                {
                    ...
                    DbCommand dbcmd = BuildCommand(this.dbconold, stroreProcName, parameters);
